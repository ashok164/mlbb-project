...
const INGAME_BASE   = "https://esportsdata-sg.mobilelegends.com/battledata?authkey=1c7c9de5798a010e8f7da8ab5b82d953&battleid=";
const POSTGAME_BASE = "https://esportsdata-sg.mobilelegends.com/postdata?authkey=1c7c9de5798a010e8f7da8ab5b82d953&battleid=";
...
function askBattleId() {
  rl.question("\n🎮 Enter Battle ID: ", (id) => {
    id = id.trim();
    if (!id) { console.log("❌ Battle ID cannot be empty."); return askBattleId(); }
    BATTLE_ID    = id;
...
function writeTxtSafe(filename, value, key) {
  ...
  try { fs.writeFileSync(path.join(TXT_DIR, filename), lastKnown[key]); } catch (e) {}
}

function writeTxt(filename, value) {
  try { fs.writeFileSync(path.join(TXT_DIR, filename), String(value ?? "")); } catch (e) {}
}
...
function updateTurtle(value, stateKey, timeKey, filename, key) {
  if (value !== null && value !== undefined) {
    ...
    if (Date.now() - turtleState[timeKey] > TURTLE_TIMEOUT) turtleState[stateKey] = "0";
  }
...
function sortByRole(players) {
  return ROLE_ORDER.map(role =>
    players.find(p => roleAssignments[String(p.roleid)] === role) || emptyPlayer()
  );
}

function sortByRolePost(players) {
  return ROLE_ORDER.map(role =>
    players.find(p => roleAssignments[String(p.roleid)] === role) || emptyPostgamePlayer()
  );
}
...
function updateTextFiles(raw) {
  try {
    ...
    if (left && right) {
      ...
      try { fs.writeFileSync(path.join(TXT_DIR, "leftdiff.txt"),  lg > rg ? "+" + (lg - rg) : ""); } catch(e) {}
      try { fs.writeFileSync(path.join(TXT_DIR, "rightdiff.txt"), rg > lg ? "+" + (rg - lg) : ""); } catch(e) {}
    }
    if (left) {
      ...
      updateTurtle(left.kill_tortoise ?? left.kill_turtoise ?? null, "left_turtle",  "last_left_turtle_time",  "left_turtle.txt",  "left_turtle");
    }
    if (right) {
      ...
      updateTurtle(right.kill_tortoise ?? right.kill_turtoise ?? null, "right_turtle", "last_right_turtle_time", "right_turtle.txt", "right_turtle");
    }
  } catch (err) { console.error("Text file update error:", err.message); }
}
...
function startRefresh() {
  ...
  refreshInterval = setInterval(async () => {
    try {
      const ingameRes = await fetch(INGAME_URL, { headers: { "Authorization": "Bearer 1c7c9de5798a010e8f7da8ab5b82d953" } });
      ...
      if (state === "end" || state === "") {
        ...
        const postRes = await fetch(POSTGAME_URL, { headers: { "Authorization": "Bearer 1c7c9de5798a010e8f7da8ab5b82d953" } });
        const postRaw = await postRes.json();
        lastPostgameRaw = postRaw;
        postgameData  = cleanPostgame(postRaw);
        cachedData    = postgameData;
        // ingameData is NOT overwritten — keeps last ingame snapshot
        ...
        writeTxt("timer.txt", "00:00"); writeTxt("left_gold.txt", "0"); writeTxt("right_gold.txt", "0");
        writeTxt("leftdiff.txt", ""); writeTxt("rightdiff.txt", ""); writeTxt("left_score.txt", "0");
        writeTxt("right_score.txt", "0"); writeTxt("left_lord.txt", "0"); writeTxt("right_lord.txt", "0");
        writeTxt("left_tower.txt", "0"); writeTxt("right_tower.txt", "0");
        writeTxt("left_turtle.txt", "0"); writeTxt("right_turtle.txt", "0");
        Object.keys(lastKnown).forEach(k => { lastKnown[k] = k.includes("diff") ? "" : "0"; });
        broadcastWsPayloads();
...
function buildIngameFallback(pgData) {
  const remap = (p) => ({
    ...
    draft_hero_image:  p.heroid ? `http://localhost:${PORT}/drafthero-image/${String(p.heroid).trim()}.png` : ""
  });
...
function normalizeWsEndpoint(endpoint) {
  const normalized = String(endpoint || "data").replace(/^\/+/, "").replace(/\/+$/, "");
  return WS_ENDPOINTS.has(normalized) ? normalized : "data";
}

function getWsPayload(endpoint) {
  const d = ingameData || (postgameData ? buildIngameFallback(postgameData) : null);

  switch (endpoint) {
    case "data":
      return cachedData ? { ok: true, data: cachedData } : { ok: false, error: "Data not ready yet" };
    case "ingame":
      return ingameData ? { ok: true, data: ingameData } : { ok: false, error: "No ingame data yet" };
    case "postgame":
      return postgameData ? { ok: true, data: postgameData } : { ok: false, error: "No postgame data yet" };
    case "players":
      return d ? { ok: true, data: d.all_players } : { ok: false, error: "Data not ready yet" };
    case "players/left":
      return d ? { ok: true, data: d.left_team.players } : { ok: false, error: "Data not ready yet" };
    case "players/right":
      return d ? { ok: true, data: d.right_team.players } : { ok: false, error: "Data not ready yet" };
    case "postgame/players":
      return postgameData ? { ok: true, data: postgameData.all_players } : { ok: false, error: "No postgame data yet" };
    case "postgame/players/left":
      return postgameData ? { ok: true, data: postgameData.left_team.players } : { ok: false, error: "No postgame data yet" };
    case "postgame/players/right":
      return postgameData ? { ok: true, data: postgameData.right_team.players } : { ok: false, error: "No postgame data yet" };
    case "golddiff":
      return cachedData?.gold_diff ? { ok: true, data: cachedData.gold_diff } : { ok: false, error: "No gold diff data yet" };
    case "postgame/golddiff":
      return postgameData?.gold_diff ? { ok: true, data: postgameData.gold_diff } : { ok: false, error: "No postgame gold diff yet" };
    default:
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", (message) => {
    try {
      ...
      ws.send(JSON.stringify({ ok: false, error: "Invalid websocket message" }));
    }
  });
});
...
app.get("/raw", async (req, res) => {
  if (!INGAME_URL) return res.status(503).json({ error: "Battle ID not set yet" });
  try { const r = await fetch(INGAME_URL, { headers: { "Authorization": "Bearer 1c7c9de5798a010e8f7da8ab5b82d953" } }); res.json(await r.json()); }
  catch (err) { res.status(500).json({ error: err.message }); }
});

app.get("/raw/post", async (req, res) => {
  if (!POSTGAME_URL) return res.status(503).json({ error: "Battle ID not set yet" });
  try { const r = await fetch(POSTGAME_URL, { headers: { "Authorization": "Bearer 1c7c9de5798a010e8f7da8ab5b82d953" } }); res.json(await r.json()); }
  catch (err) { res.status(500).json({ error: err.message }); }
});

app.get("/data",     (req, res) => { if (cachedData)    res.json(cachedData);              else res.status(503).json({ error: "Data not ready yet" }); });
app.get("/ingame",   (req, res) => { if (ingameData)    res.json(ingameData);              else res.status(503).json({ error: "No ingame data yet" }); });
app.get("/postgame", (req, res) => { if (postgameData)  res.json(postgameData);            else res.status(503).json({ error: "No postgame data yet" }); });
...
app.get("/players", (req, res) => {
  const d = ingameData || (postgameData ? buildIngameFallback(postgameData) : null);
  if (d) res.json(d.all_players); else res.status(503).json({ error: "Data not ready yet" });
});
app.get("/players/left", (req, res) => {
  const d = ingameData || (postgameData ? buildIngameFallback(postgameData) : null);
  if (d) res.json(d.left_team.players); else res.status(503).json({ error: "Data not ready yet" });
});
app.get("/players/right", (req, res) => {
  const d = ingameData || (postgameData ? buildIngameFallback(postgameData) : null);
  if (d) res.json(d.right_team.players); else res.status(503).json({ error: "Data not ready yet" });
});

// ✅ /postgame/players — always postgameData — hero_image = /images/postheros/
app.get("/postgame/players",       (req, res) => { if (postgameData) res.json(postgameData.all_players);        else res.status(503).json({ error: "No postgame data yet" }); });
app.get("/postgame/players/left",  (req, res) => { if (postgameData) res.json(postgameData.left_team.players);  else res.status(503).json({ error: "No postgame data yet" }); });
app.get("/postgame/players/right", (req, res) => { if (postgameData) res.json(postgameData.right_team.players); else res.status(503).json({ error: "No postgame data yet" }); });

app.get("/golddiff",         (req, res) => { if (cachedData?.gold_diff)   res.json(cachedData.gold_diff);   else res.status(503).json({ error: "No gold diff data yet" }); });
app.get("/postgame/golddiff",(req, res) => { if (postgameData?.gold_diff) res.json(postgameData.gold_diff); else res.status(503).json({ error: "No postgame gold diff yet" }); });
...
app.get("/drafthero-image/:heroid", (req, res) => {
  const heroid = req.params.heroid.replace(/\.png$/i, "").replace(/\.jpg$/i, "");
...
app.get("/emblem-image/:runeid",  (req, res) => { const p = `C:/Users/User/Desktop/vmixData/images/emblems/${req.params.runeid}.png`;              fs.existsSync(p) ? res.sendFile(p) : res.status(404).send("Not found"); });
app.get("/talent-image/:talentid",(req, res) => { const p = `C:/Users/User/Desktop/vmixData/images/emblems/talents/${req.params.talentid}.png`;    fs.existsSync(p) ? res.sendFile(p) : res.status(404).send("Not found"); });
app.get("/spell-image/:skillid",  (req, res) => { const p = `C:/Users/User/Desktop/vmixData/images/spells/${req.params.skillid}.png`;              fs.existsSync(p) ? res.sendFile(p) : res.status(404).send("Not found"); });
app.get("/playerpic/:roleid",     (req, res) => { const p = `C:/Users/User/Desktop/vmixData/images/playerpic/${req.params.roleid}.png`;            fs.existsSync(p) ? res.sendFile(p) : res.status(404).send("Not found"); });
app.get("/txt/:filename",         (req, res) => { const f = path.join(TXT_DIR, req.params.filename); fs.existsSync(f) ? res.sendFile(f) : res.status(404).send("Not found"); });

app.get("/assign", (req, res) => {
  if (!cachedData) return res.send(`<html><body style="background:#1a1a2e;color:white;font-family:Arial;padding:30px;"><h2>⏳ Data not ready yet. Please refresh.</h2></body></html>`);
  const roles = ["exp", "mid", "roam", "jungle", "gold"];
  const makeRows = (players) => players.filter(p => p.heroid).map(p => `
    <tr>
...
function cleanIngamePlayer(p) {
  ...
  return {
    ...
    draft_hero_image:    p.heroid ? `http://localhost:${PORT}/drafthero-image/${String(p.heroid).trim()}.png` : "",       // ✅ /images/drafthero-image/ with .png
    campid:              p.campid              ?? "",
    pos:                 p.pos                 ?? 0,
...
function emptyPlayer() {
  return {
    name: "", role: "unassigned", roleid: "", playerpic_image: "", heroid: "",
    hero_image: "", ingame_hero_image: "", draft_hero_image: "", campid: "", pos: 0,
    kill_num: 0, dead_num: 0, assist_num: 0, kda: "0 / 0 / 0",
    gold: 0, gold_k: "0", max_hp: 0, cur_hp: 0, hp_pct: 0, hp_bar: "0%",
    max_mana: 0, cur_mana: 0, mana_pct: 0, mana_bar: "0%",
    dead: false, revive_left_time: 0, level: 0,
    total_damage: 0, total_hurt: 0, total_heal: 0, total_damage_tower: 0, total_heal_other: 0,
    xpm: 0, skillid: "", spell_image: "", skill_left_time: 0, control_time_ms: 0,
    physical_defense: 0, magic_defense: 0, map_pos_x: 0, map_pos_y: 0,
    rune_id: "", rune_image: "", rune_talent_1: "", rune_talent_2: "", rune_talent_3: "",
    rune_talent_1_image: "", rune_talent_2_image: "", rune_talent_3_image: "",
    left_gold_diff: "", right_gold_diff: "", gold_leader: "even",
    equips: Array.from({ length: 6 }, () => ({ id: "blank", image: itemImg(null) }))
  };
}
...
function cleanIngame(raw) {
  ...
  leftPlayers  = leftPlayers.map((p, i) => ({ ...p, left_gold_diff: goldDiff[ROLE_ORDER[i]]?.left_gold_diff ?? "", right_gold_diff: goldDiff[ROLE_ORDER[i]]?.right_gold_diff ?? "", gold_leader: goldDiff[ROLE_ORDER[i]]?.leader ?? "even" }));
  rightPlayers = rightPlayers.map((p, i) => ({ ...p, left_gold_diff: goldDiff[ROLE_ORDER[i]]?.left_gold_diff ?? "", right_gold_diff: goldDiff[ROLE_ORDER[i]]?.right_gold_diff ?? "", gold_leader: goldDiff[ROLE_ORDER[i]]?.leader ?? "even" }));

  return {
    state: "ingame", game_time: gameTime, game_time_fmt: formatTime(gameTime),
    gold_diff: { exp: goldDiff["exp"], mid: goldDiff["mid"], roam: goldDiff["roam"], jungle: goldDiff["jungle"], gold: goldDiff["gold"] },
    left_team:  { name: leftCamp.team_name  ?? "LEFT",  total_kills: leftKills,  kill_lord: leftCamp.kill_lord  ?? 0, kill_tower: leftCamp.kill_tower  ?? 0, kill_tortoise: leftCamp.kill_tortoise  ?? 0, players: leftPlayers  },
    right_team: { name: rightCamp.team_name ?? "RIGHT", total_kills: rightKills, kill_lord: rightCamp.kill_lord ?? 0, kill_tower: rightCamp.kill_tower ?? 0, kill_tortoise: rightCamp.kill_tortoise ?? 0, players: rightPlayers },
...
function cleanPostgamePlayer(p) {
  ...
  return {
    ...
    assist_num:          p.assist_num          ?? 0,
    kda:                 `${p.kill_num ?? 0} / ${p.dead_num ?? 0} / ${p.assist_num ?? 0}`,
    total_money:         p.total_money         ?? 0,
    total_money_k:       formatGold(p.total_money),
...
function emptyPostgamePlayer() {
  return {
    name: "", role: "unassigned", roleid: "", playerpic_image: "", heroid: "",
    hero_image: "", draft_hero_image: "", campid: "",
    kill_num: 0, dead_num: 0, assist_num: 0, kda: "0 / 0 / 0",
    total_money: 0, total_money_k: "0", min_money: 0,
    total_damage: 0, total_hurt: 0, total_heal: 0, total_damage_tower: 0, total_heal_other: 0, min_damage: 0,
    level: 0, score: 0, mvp: false, money_percent: 0, offered_rate: 0,
    skillid: "", spell_image: "", skill_use_times: 0, xpm: 0,
    rune_id: "", rune_image: "", rune_talent_1: "", rune_talent_2: "", rune_talent_3: "",
    rune_talent_1_image: "", rune_talent_2_image: "", rune_talent_3_image: "",
    left_gold_diff: "", right_gold_diff: "", gold_leader: "even",
    equips: Array.from({ length: 6 }, () => ({ id: "blank", image: itemImg(null) }))
...
function cleanPostgame(raw) {
  ...
  let leftPlayers  = heroList.filter(p => p.campid === 1).map(cleanPostgamePlayer);
  let rightPlayers = heroList.filter(p => p.campid === 2).map(cleanPostgamePlayer);
  ...
  const leftCamp  = campList.find(c => c.campid === 1) || {};
  const rightCamp = campList.find(c => c.campid === 2) || {};
  const leftGoldRaw  = leftPlayers.reduce((s, p)  => s + (Number(p.total_money) || 0), 0);
  const rightGoldRaw = rightPlayers.reduce((s, p) => s + (Number(p.total_money) || 0), 0);
  const leftKills    = rightPlayers.reduce((s, p) => s + (Number(p.dead_num) || 0), 0);
  const rightKills   = leftPlayers.reduce((s, p)  => s + (Number(p.dead_num) || 0), 0);
  ...
  return {
    state: "postgame", game_time: gameTime, game_time_fmt: formatTime(gameTime), win_camp: winCamp,
    gold_diff: { exp: goldDiff["exp"], mid: goldDiff["mid"], roam: goldDiff["roam"], jungle: goldDiff["jungle"], gold: goldDiff["gold"] },
    left_team:  { name: leftCamp.team_name  ?? "LEFT",  win: winCamp === 1, total_kills: leftKills,  total_gold: formatGold(leftGoldRaw),  kill_lord: leftCamp.kill_lord  ?? 0, kill_tower: leftCamp.kill_tower  ?? 0, kill_tortoise: leftCamp.kill_totoise  ?? 0, players: leftPlayers  },
    right_team: { name: rightCamp.team_name ?? "RIGHT", win: winCamp === 2, total_kills: rightKills, total_gold: formatGold(rightGoldRaw), kill_lord: rightCamp.kill_lord ?? 0, kill_tower: rightCamp.kill_tower ?? 0, kill_tortoise: rightCamp.kill_totoise ?? 0, players: rightPlayers },
    all_players: [...leftPlayers, ...rightPlayers]
  };
}
...
server.listen(PORT, () => {
  ...
  console.log(`\n🔄  LIVE / COMBINED`);
  console.log(`   ${B}/data                     → current state (ingame or postgame)`);
  ...
  console.log(`   ${B}/drafthero-image/:heroid.png → draft hero image (.png supported)`);
